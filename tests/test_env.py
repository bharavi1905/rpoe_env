...
def test_reset_returns_observation(env):
    ...
    assert obs.wheel_count == 7
    assert len(obs.wheels) == 7
    assert len(obs.slots) == 84
    assert obs.step == 0
...
def test_step_returns_correct_types(env):
    env.reset()
    obs = env.step(RPOEAction(action=ActionType.ROTATE_CW, wheel_index=0))
    assert isinstance(obs, RPOEObservation)
...
def test_rotate_cw_moves_wheel(env):
    ...
    env.step(RPOEAction(action=ActionType.ROTATE_CW, wheel_index=0))
    s = env.state
...
def test_rotate_ccw_is_inverse_of_cw(env):
    ...
    env.step(RPOEAction(action=ActionType.ROTATE_CW, wheel_index=0))
    env.step(RPOEAction(action=ActionType.ROTATE_CCW, wheel_index=0))
...
def test_illegal_park_penalised(env):
    ...
    env._slots[0][0] = "TEST-CAR"
    env._arrival_q.clear()
    # Arrival queue is empty AND slot occupied — park is illegal
    env.step(RPOEAction(action=ActionType.ROTATE_CW, wheel_index=0))  # just rotate
    # Now try park when queue is empty
    obs2 = env.step(RPOEAction(action=ActionType.PARK, wheel_index=0))
...
def test_same_seed_same_trajectory():
    ...
    for rewards in [rewards_a, rewards_b]:
        ...
        rnd.seed(99)
        for _ in range(20):
            act = RPOEAction(action=rnd.choice(list(ActionType)), wheel_index=0)
            obs = e.step(act)
...
def test_multiwheel_rotation_is_isolated():
    env = RotaryParkingEnv(seed=42, wheel_count=2, wheel_size=4)
    env.reset()
    env._slots[0][0] = "W0"
    env._slots[1][0] = "W1"

    env.step(RPOEAction(action=ActionType.ROTATE_CW, wheel_index=1))

    assert env._slots[0] == ["W0", None, None, None]
    assert env._slots[1] == [None, "W1", None, None]
